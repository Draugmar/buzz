...
- `name_color` on a `ManagedAgentRecord` is a create-time snapshot from its linked persona, but unlike `system_prompt`/`model`/`provider` — which are also pinned onto the record for display/backward-compat, yet are resolved LIVE from the definition on every spawn/deploy (see `effective_config::resolve_effective_config`) and so do propagate on the next restart — `name_color` has no live-read path at all. It is the one field that genuinely never propagates after create time short of delete+respawn. This is a deliberate simplification: `name_color` is a local UI-only value, so there is no relay-consistency reason to force a sync.
